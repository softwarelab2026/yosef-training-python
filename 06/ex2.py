...
def match_ends(words):
    count_of_matching_words = 0
    for word in words:
        if valid_word(word):
            count_of_matching_words += 1
    return count_of_matching_words
...
def remove_adjacent(nums):
    # I know it's easy to do by creating a new list.
    # I wanted to try with one list.
    i = 0
    while i < len(nums) -1:
        if nums[i] == nums[ i + 1]:
            nums.pop(i + 1)
